...
`RaceMapSurface` places one transparent SceneKit scene above an `MKMapView`. Four MapKit ground samples define a shared Mercator-to-screen projective transform whenever the camera changes. Car coordinates and orientation vectors then use that cached transform, including map heading and pitch. A vertical orthographic camera aligns the scene with the map's logical pixels on both platforms. Cars use the bundled 2026 concept model (see Credits); the original procedural meshes with bodywork, wings, open wheels, suspension, cockpit, and halo remain as a fallback.
