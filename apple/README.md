...
- Choose a venue on the globe, a race-weekend date in the month calendar, or a track in the ordered schedule. Clicking a city pin selects that venue without flying the jet; clicking the selected pin again (or **Fly over circuit** on the race card) runs a cinematic pass: the camera settles above the track, chases one lap along the centerline, then pulls back to a three-quarter view. Any new selection ends the pass. **Open race** loads that circuit’s simulated replay. Swipe left/right across the month calendar to advance or go back a month; the arrow buttons remain available. **Entire globe** resets the camera. Calendar controls remain visible while browsing.
- Picking a neighboring round from a city pin, the list, or month grid flies a private jet directly between those two cities along their geodesic route, in either direction. Selecting a nonadjacent round cancels any flight and moves directly to that track without a jet or intermediate stops. Flight time is compressed to one second per hour at 900 km/h, with a three-second minimum per leg so short hops remain visible. The camera first frames departure over 0.8 seconds, then follows the exact same constant-speed geodesic position as the G650 on every display frame. It holds its arrival camera without a second zoom. Selecting a neighboring round mid-flight redirects from the displayed jet position; Entire globe or leaving Calendar cancels the trip. The camera follows the jet, a red trail follows the same geographic path and ends at the jet’s current position, and the race card shows the leg from the previous round with its distance and flight time. Reduce Motion skips the flight. The jet is the bundled Gulfstream G650ER model (see Credits).
