...
struct DriverRecording: Codable {
    ...
    func position(at time: Double) -> CarPosition {
        ...
        return CarPosition(driver: driver, point: spatialA.point.interpolated(to: spatialB.point, fraction: spatialFraction),
                           ...
                           gear: fraction < 1 ? a.gear : b.gear, rpm: fraction < 1 ? a.rpm : b.rpm)
    }
}
    ...
    func validated() throws -> RaceReplay {
        ...
        for recording in recordings {
            ...
            for sample in recording.samples {
                ...
                guard sample.gear.map({ (0...8).contains($0) }) ?? true,
                      sample.rpm.map({ (0...25_000).contains($0) }) ?? true else {
                    throw ReplayError.invalid("Telemetry needs gears 0–8 and RPM 0–25,000.")
                }
...
extension DemoCircuit {
    func loadCircuit() throws -> [GeoPoint] {
        ...
        return CircuitRoute.smoothed(points: points).points
    }

    func load() throws -> RaceReplay {
        let route = CircuitRoute(points: try loadCircuit())
...
struct CircuitRoute {
    ...
    /// Splits the route into equal-length segments starting at the first circuit point,
    /// with interpolated boundary points — the same thirds RaceTiming uses for sectors.
    func sectors(_ count: Int = 3) -> [[GeoPoint]] {
        guard count > 1, length > 0 else { return [points] }
        return (0..<count).map { index in
            let start = length * Double(index) / Double(count)
            let end = length * Double(index + 1) / Double(count)
            var segment = [point(at: start)]
            for (i, point) in points.enumerated() where distances[i] > start && distances[i] < end {
                segment.append(point)
            }
            segment.append(point(at: end))
            return segment
        }
    }
}
