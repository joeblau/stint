...
final class MapFlowTests: XCTestCase {
    ...
    @MainActor func testPinchReleasesFollowCamera() async throws {
        ...
        let clearMap = expectation(for: NSPredicate(format: "exists == false"), evaluatedWith: app.buttons["tab-calendar"])
        ...
        add(zoomedOut)
        let clearAgain = expectation(for: NSPredicate(format: "exists == false"), evaluatedWith: app.buttons["tab-calendar"])
