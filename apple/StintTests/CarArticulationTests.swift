...
final class CarArticulationTests: XCTestCase {
    ...
    func testVisualSteeringGainRemainsBounded() {
        let rig = CarGeometry.make(color: "#E10600")
        rig.apply(.init(frontLeft: 1, frontRight: -1, drsOpen: false), animated: false)
        XCTAssertEqual(rig.frontLeft.simdEulerAngles.y, Float(CarGeometry.Rig.steeringLimit), accuracy: 0.0001)
        XCTAssertEqual(rig.frontRight.simdEulerAngles.y, -Float(CarGeometry.Rig.steeringLimit), accuracy: 0.0001)
    }
    ...
    func testOnlyFrontWheelsAndWingFlapArticulate() throws {
        ...
        XCTAssertEqual(rig.frontLeft.simdEulerAngles.y, -0.36, accuracy: 0.0001)
        XCTAssertEqual(rig.frontRight.simdEulerAngles.y, -0.432, accuracy: 0.0001)
