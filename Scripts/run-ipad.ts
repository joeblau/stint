...
async function main() {
  ...
  try {
    ...
    await $`xcodebuild -quiet -project Stint.xcodeproj -scheme Stint-iPad -configuration Debug -destination ${`platform=iOS,id=${udid}`} -destination-timeout 30 -derivedDataPath build-device -allowProvisioningUpdates -allowProvisioningDeviceRegistration CODE_SIGN_STYLE=Automatic ${signing} build`;
    await $`xcrun devicectl device install app --device ${udid} build-device/Build/Products/Debug-iphoneos/Stint.app --timeout 60`;
