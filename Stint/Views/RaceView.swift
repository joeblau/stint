...
struct RaceView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            ZStack {
                RaceMapView(session: session, frameTime: session.time, active: mode == .race || transitioning)
                    .ignoresSafeArea()
        ...
        .task(id: scenePhase) {
            ...
            while !Task.isCancelled {
                ...
                if mode == .race && !transitioning { session.advance(by: now - previous) }
                ...
                session.overlays.update(at: now, isInteracting: showsSettings || showsLighting || showsImporter
                                        || mode == .calendar || transitioning || isScrubbing || session.importing || session.error != nil || voiceOverEnabled, autoHideEnabled: autoHideHUD)
                previous = now
            }
        }
    }
    ...
    private func openRace(_ circuit: DemoCircuit) {
        ...
        transitionTask = Task { @MainActor in
            do {
                let replay = try await Task.detached(priority: .userInitiated) { try circuit.load() }.value
                guard !Task.isCancelled else { return }
                session.install(replay, source: "SIMULATED")
                ...
                VStack(spacing: 3) {
                    ForEach(session.standings) { car in
                        Button {
                            ...
                            .background(.white.opacity(car.id == session.selectedDriverID ? 0.16 : 0),
    ...
    private var playback: some View {
        HStack(spacing: 12) {
            ...
            Slider(value: $session.time, in: 0...max(1, session.duration), onEditingChanged: { editing in
                session.overlays.reveal()
                isScrubbing = editing
                if editing {
...
private struct GlassPanel<S: InsettableShape>: ViewModifier {
    ...
    @ViewBuilder func body(content: Content) -> some View {
        if #available(macOS 26.0, iOS 26.0, *) {
            content
                .foregroundStyle(StintPalette.white)
                .glassEffect(.regular.tint(StintPalette.asphalt.opacity(0.06)), in: shape)
        } else {
            content
                .foregroundStyle(StintPalette.white)
                .background(.ultraThinMaterial, in: shape)
                .overlay(shape.strokeBorder(StintPalette.white.opacity(0.16), lineWidth: 1))
        }
    }
}
