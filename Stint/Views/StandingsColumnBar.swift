...
struct StandingsColumnBar: View {
    ...
    private func icon(_ symbol: String, active: Bool) -> some View {
        Image(systemName: symbol)
            ...
            .foregroundStyle(active ? StintPalette.white : .secondary)
            .frame(maxWidth: .infinity)
            .frame(height: compact ? 28 : 34)
            .background(.white.opacity(active ? 0.18 : 0.06), in: RoundedRectangle(cornerRadius: 9))
            ...
            List {
                ForEach(Array(order.enumerated()), id: \.element) { index, column in
                    ...
                    HStack(spacing: 10) {
                        ...
                        Spacer(minLength: 6)
                        if isSelected {
                            Image(systemName: "checkmark").font(.system(size: 11, weight: .bold)).foregroundStyle(StintPalette.white)
                        }
                    ...
                    .listRowBackground(RoundedRectangle(cornerRadius: 8).fill(.white.opacity(isSelected ? 0.14 : (isFavorite ? 0.04 : 0))))
...
struct StandingsCell: View {
    ...
    private func plain(_ value: String) -> some View {
        Text(value).foregroundStyle(isLeader ? StintPalette.white : .secondary)
    }
