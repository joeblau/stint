...
struct SeasonCalendarView: View {
                    ...
                    HStack(alignment: .top, spacing: 12) {
                        ...
                        VStack(alignment: .trailing, spacing: 12) {
                            ...
                            Button {
                                ...
                                Label("Import JSON replay", systemImage: "square.and.arrow.down")
                                    .font(.system(size: 12, weight: .medium)).padding(14)
                                    .contentShape(Rectangle())
                            }
                            .glassPanel()
                            .accessibilityIdentifier("import-replay")
                            if let selection {
                        ...
                        ForEach(Season2026.races) { race in
                            Button { select(race) } label: {
                                HStack(spacing: 10) {
                                    ...
                                    Spacer(minLength: 0)
                                    Image(systemName: race.isCompleted(at: now) ? "checkmark.circle.fill" : "circle")
                                        .font(.system(size: 11)).foregroundStyle(race.isCompleted(at: now) ? StintPalette.white : .secondary)
                                }
    ...
    private func detail(_ race: SeasonRace, now: Date) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text("Simulated replay").font(.system(size: 9)).foregroundStyle(.tertiary)
        }
